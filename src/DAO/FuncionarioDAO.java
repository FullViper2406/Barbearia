...
public class FuncionarioDAO {
    ...
    public void cadastar(FuncionarioDTO objfuncionariodto) {
        String sql = "insert into funcionario (Nome,Id,Cpf,Contato,Endereco,Senha) values(?,?,?,?,?,?)";
    ...
    public ArrayList<FuncionarioDTO> PesquisarFuncionario() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                objfuncionariodto.setCodigo(rs.getString("Id"));
    ...
    public void alterar(FuncionarioDTO objfuncionariodto) {
        String sql = "update  funcionario set  Nome = ? ,Id = ?,Cpf = ?,Contato = ?,Endereco = ?,where Senha = ?";
        conn = new ConexaoDAO().conectaBD();
        try {
            pstm = conn.prepareStatement(sql);
            pstm.setString(1, objfuncionariodto.getNome());
